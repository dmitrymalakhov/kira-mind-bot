...
export async function quickFactCheck(message: string): Promise<QuickFact[]> {
    ...
    const prompt = `
Определи, содержит ли сообщение пользователя ЯВНЫЕ личные факты о нем (имя, возраст, отношения, семья, личные данные, устойчивые предпочтения, работа, место жительства).
