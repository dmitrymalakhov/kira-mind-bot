...
const EXPLICIT_REMEMBER_PATTERNS: RegExp[] = [
    ...
    /сохрани\s+в память\s*(?:(?:,\s*)?что|,|:)\s*(.+)/i,
    /сохрани\s+((?:эти\s+)?(?:факты?|данные|сведения|информацию?|инфу)\s+(?:об?\s+|про\s+).+)/i,
    /сохрани\s*(?:(?:,\s*)?что|,|:)\s*(.+)/i,
    // «Запиши что …» / «Запиши, что …»
    /запиши\s*(?:(?:,\s*)?что|,)\s*(.+)/i,
    // «Не забывай что …» / «Не забывай, что …»
    /не забывай\s*(?:(?:,\s*)?что|,)\s*(.+)/i,
    // «Важно запомнить что …» / «Нужно запомнить …»
    /(?:важно|нужно)\s+запомнить\s*(?:,\s*)?(?:что\s+)?(.+)/i,
    // «Хочу чтобы ты запомнила …» / «Запомни пожалуйста что …»
    /(?:хочу\s+чтобы\s+ты\s+запомнила?|запомни\s+пожалуйста)\s*(?:,\s*)?(?:что\s+)?(.+)/i,
    // English
    /remember\s+that\s+(.+)/i,
