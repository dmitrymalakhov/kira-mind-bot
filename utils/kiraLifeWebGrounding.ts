...
export function buildKiraLifeWebGroundingQuery(input: KiraLifeWebGroundingQueryInput): string {
  ...
  return [
    ...
    `Текущее место жизни персонажа: ${compactContext(input.currentResidence, 200) || "не задано"}. Это авторитетный текущий город; место происхождения и старые главы биографии не заменяют его.`,
    "",
    ...
    "",
    "Ограничения:",
    "- если опора локальная, используй только явно указанное текущее место жизни; город происхождения и прошлые места из биографии не считай текущими; не выбирай произвольный город и не делай каждую опору локальной;",
