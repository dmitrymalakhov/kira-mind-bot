 ...
 *
 * false означает: запись описывает контакт/чужой чат и не должна попадать
...
export function isEligibleForUserSynthesis(memory: UserSynthesisSourceLike): boolean {
    // 1. Контактные факты — даже корректно размеченные — исключаем.
    if (memory.subject === 'contact') return false;

    const tags = memory.tags ?? [];

    // 2. Эпизоды фоновой рефлексии/personal-chat про чужой чат: их subject
