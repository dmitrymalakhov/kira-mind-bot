...
export function getCapabilitiesMessage(): string {
    return [
        ...
        "",
        "Коротко: умею ставить и менять напоминания, помнить факты, искать в интернете, работать с картами, анализировать фото, генерировать картинки, читать и анализировать Telegram-переписки, готовить сообщения, вести переговоры и выполнять некоторые задачи в браузере.",
        "",
        "Спроси, например:",
        "• «Ты можешь изучить переписку с человеком?»",
        "• «Как попросить тебя отправить сообщение маме?»",
