...
- Переключение Gemini на lite-модель теперь видно в Docker-консоли как две связанные однострочные записи со стабильным маркером `[AI DEGRADED]`: событие `start` показывает маршрут `from → to` и причину, а `success` подтверждает, что lite-модель действительно ответила, с фактической моделью и задержкой.
