...
- Полные Telegram text+markup edits и отдельные markup-only edits теперь проходят через одну очередь на сообщение, поэтому конкурентный старый запрос не может перезаписать более новую клавиатуру.
