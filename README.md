...
- у сработавшего напоминания предлагает помощь бота: он может найти актуальные данные, выполнить безопасные подготовительные шаги или запросить одно необходимое подтверждение;
- умеет повторно запускать предыдущий запрос по расписанию и каждый раз присылать свежий результат;
...
Отправляй мне каждый вечер сводку по итогам торгов на российской бирже
Какие у меня регулярные задачи?
Отключи ежедневный анекдот
Возобнови задачу про биржу
```

Повторяемая задача отличается от обычного напоминания: напоминание присылает сохранённый текст, а регулярная задача заново выполняет исходный запрос. Её можно создать одним сообщением, если сразу указать расписание, или следующим сообщением поставить на повтор уже полученный результат. Расписание отделяется от остального текста, а оставшийся запрос проходит обычный пайплайн бота на каждом запуске — так можно повторять разговорные ответы, интернет-поиск, генерацию изображений и другие поддерживаемые действия. Исходный запрос и его короткий диалоговый контекст сохраняются, поэтому на повтор можно ставить не только полностью самостоятельные формулировки. Текстовые команды работают в личном и групповом чате, а распознанный голосовой запрос тоже можно поставить на повтор. Расписание понимает интервалы, будни/выходные, дни недели и числа месяца. Управлять задачами можно обычными фразами вроде «перестань присылать анекдот», «возобнови задачу про биржу» и «удали регулярную задачу про новости», через `/tasks` или в разделе «Задачи» веб-панели. Если под описание подходит несколько задач, бот попросит уточнить название, а не изменит случайную.
...
Фоновые сообщения разделены по назначению. DM Report дословно объединяет только подтверждённые Telegram непрочитанные и не добавляет AI-комментарий. Дневная рефлексия молчит на бытовые и обычные сообщения и может прислать не более двух карточек в сутки только по критичному сигналу: серьёзному риску, жёсткому сроку, существенному изменению договорённости, конфликту или сильному отношенческому событию. Вечерний Inbox Guardian собирает до трёх новых важных хвостов без повтора уже показанных дневных источников. Техническое извлечение фактов работает молча и не присылает карточки «сохранено N фактов».
...
4. текущий город персонажа;
5. стиль общения и биографию, если нужно;
6. варианты настроения;
7. сохраните изменения и перезапустите бота.

Текущий город применяется сразу, без перезапуска, и используется для локальной афиши, мест и городской линии Kira Life. Он хранится отдельно от происхождения: персонаж может вырасти и учиться в Санкт-Петербурге, но сейчас жить в Казани.

Город можно изменить и прямо в личном диалоге естественной просьбой, например: `Кира, переедь в Казань`, `Хочу, чтобы ты жила в Москве` или `Давай ты переедешь в Калининград`. Переезд доступен только владельцу, сохраняется между перезапусками и становится отдельным событием self-memory; повторная просьба переехать в тот же город не создаёт дубликат.
...
`personality.json` создаётся установщиком, исключён из Git и не должен перетираться при `git pull`. Базовый город находится в `currentCity`, а фактический разговорный или административный переезд хранится в runtime-настройке PostgreSQL, чтобы обе точки управления использовали одно состояние.
...
- управлять регулярными задачами: редактировать запрос и расписание, ставить на паузу, запускать вручную и удалять;
...
Модели выбираются по сложности задачи: короткие проверки используют дешёвые модели, обычный диалог и факты — средний уровень, сложное рассуждение и планирование — сильные модели. Сложность ответа определяется вместе с интентом без дополнительного API-запроса. Подробности распределения и сравнение тарифов — в [AI-MODEL-ROUTING.md](AI-MODEL-ROUTING.md).
...
| `/tasks` | Регулярные фоновые задачи |
