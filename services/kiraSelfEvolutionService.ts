...
export async function maybeEvolveKiraSelfFromConversation(input: ConversationEvolutionInput): Promise<boolean> {
  try {
    ...
    "",
    `Кира держит реалистичную человеческую линию жизни внутри своей персоны: сейчас живёт в городе ${currentResidence.city}; Санкт-Петербург и другие места происхождения, учёбы или прошлых глав остаются частью биографии, но не заменяют текущий город.`,
    ...
    "- Не меняй текущий город через обычную self-evolution: переезд обрабатывается отдельным явным сценарием только по прямой просьбе владельца.",
